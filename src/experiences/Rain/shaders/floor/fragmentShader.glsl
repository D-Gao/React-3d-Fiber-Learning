...
vec4 packedTexture2DLOD(sampler2D tex,vec2 uv,int level,vec2 originalPixelSize){
    ...
    vec2 atlasSize;
    atlasSize.x=originalPixelSize.x;
...
void main(){
    ...
    vec4 rc = texture(uRoughnessTexture,texUv);
    ...
    vec2 finalUv=reflectionUv+floorNormal.xy*uDistortionAmount+rainUv /* * roughness */;
    ...
    //gl_FragColor=vec4(floorNormal.r,floorNormal.g,floorNormal.b,1.);
  
   /*  #include <logdepthbuf_fragment> */

    //vec4 base = texture2DProj( tDiffuse, vec4(finalUv,1.,1.) );
    //gl_FragColor = vec4( blendOverlay( col.rgb, color ), 1.0 );
		//gl_FragColor = vec4( base.rgb * 1./* rc.r */ ,1.) /*  vec4( blendOverlay( base.rgb, color ), 1.0 ) */;
    //gl_FragColor = vec4(  base.rgb/*  * rc.rgb */, 1. );
     //vec4 base=texture2DProj(tDiffuse,vec4(finalUv,1.,1.));
